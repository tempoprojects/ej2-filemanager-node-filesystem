import { isAuthenticated, getProject } from './parse-helpers';
import { Request, Response } from 'express'

export const isValidInstallationId = async (installationId: string, res: Response) => {
    if (!(await isAuthenticated(installationId as string))) {
...
export const assertProject = async (req: Request, res: Response) => {
    ...
    const qs = req.query;
    // extract Parse Installation ID & Project shortCode from the query string
    const installationId = qs.installationId as string;
    const projectShortCode = qs.projectShortCode as string;

    if (!await isValidInstallationId(installationId, res)) {
        return;
    }

    const project = await getProject(projectShortCode);
